...
function chooseBestSum(t, k, ls) {
 ...
 let answers = [];
  ...
  function rabbitHole(arr,setLimit, target, dists) {
    ...
    for (let i =0; i <= dists.length - setLimit; i++) {
        arr.push(dists[i]);
        rabbitHole(arr,setLimit-1, target, dists.slice(i+1));
        arr.pop();
    }
  return answers
}
  
  rabbitHole([], k, t, dists);
  
  answers = answers.filter(x => x <= t);
  return answers.length >= 1? Math.max(...answers) : null
